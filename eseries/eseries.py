...
class EBase:
    ...
    def __init__(self, name: str, tolerance: float, base_exponent: int, base_values: Tuple[int, ...]):
        ...
        EBase._instances.append(self)
    
    @classmethod
    def get_instances(cls) -> Tuple['EBase']:
        """ Get a tuple with all the EBase instances.
    ...
    def iterate_up(self, value: float) -> Generator[float, None, None]:
        ...
        index = bisect_left(self._base_values, mantissa)
        while True:
    ...
    def __iter__(self):
        return self._base_values.__iter__()
...
E3 = EBase('E3', 0.4, 1, (10, 22, 47) )
E6 = EBase('E6', 0.2, 1, (10, 15, 22, 33, 47, 68) )
E12 = EBase('E12', 0.1, 1, (10, 12, 15, 18, 22, 27, 33, 39, 47, 56, 68, 82) )
E24 = EBase('E24', 0.05, 1, (10, 11, 12, 13, 15, 16, 18, 20, 22, 24, 27, 30, 33, 36, 39, 43, 47, 51, 56, 62, 68, 75,
                             82, 91) )
E48 = EBase('E48', 0.02, 2, (100, 105, 110, 115, 121, 127, 133, 140, 147, 154, 162, 169, 178, 187, 196, 205, 215,
...
E96 = EBase('E96', 0.01, 2, (100, 102, 105, 107, 110, 113, 115, 118, 121, 124, 127, 130, 133, 137, 140, 143, 147,
...
E192 = EBase('E192', 0.005, 2, (100, 101, 102, 104, 105, 106, 107, 109, 110, 111, 113, 114, 115, 117, 118, 120, 121,
...
def series_from_name(name: str) -> EBase:
    """Get an EBase from its name.

    Args:
        name: The series name as a string, for example 'E24'

    Returns:
        An EBase object which can be uses as a series_key.

    Raises:
        ValueError: If not such series exists.
    """
    instances = EBase.get_instances()
    try:
